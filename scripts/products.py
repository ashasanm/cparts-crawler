...
class Products():
    ...
    def get_products(self, category: str, year_filter: str = None, is_custom: bool = False, query: dict = {}):
        """ find account by query """
        if year_filter:
            return self.db[category].find({'extraction_date': {'$regex': year_filter}})
        if is_custom:
    ...
    def filter_by_marketplace(self, marketplace, category, year_filter):
